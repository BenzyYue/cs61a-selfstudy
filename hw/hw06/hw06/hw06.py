...
class VendingMachine:
    ...
    def restock(self, n: int) -> str:
        ...
        """
        "*** YOUR CODE HERE ***"
        self.stocks = self.stocks + n
        combined_string = f'Current {self.product} stock: {self.stocks}'
    ...
    def add_funds(self, n: int) -> str:
        ...
        if self.stocks == 0:
            ...
            combined_string = f'Current balance: ${self.funds}'
            return combined_string
    ...
    def vend(self) -> str:
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.funds >= self.price and self.stocks >= 1:
        ...
        elif self.funds >= self.price and self.stocks == 0:
            return f'Nothing left to vend. Please restock.'
        elif self.funds < self.price and self.stocks >= 1:
...
def store_digits(n: int):
    ...
    def helper(n, power):
        ...
        digit = n // power
        rest = n % power
        return Link(digit, helper(rest, power // 10))
...
def prune_small(t, n):
    ...
    while len(t.branches) > n:
        largest = max(t.branches, key = helper)
        for b in t.branches:
            if helper(b) == largest:
                del(b)
    for b in t.branches:

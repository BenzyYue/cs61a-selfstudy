def shuffle(s):
    ...
    for i in range(len(s)//2):
        s_first_half.append(s[i])

    for i in range(len(s)//2, len(s)):
        s_second_half.append(s[i])
    ...
    while i < len(s):
        if i % 2 == 0:
            s_final.append(s_first_half[j])
            i = i + 1
            j = j + 1
        else:
            s_final.append(s_second_half[k])
            i = i + 1
            k = k + 1
    return s_final
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    if is_planet(end(left(m))) == True and is_planet(end(right(m))) == True:
        if length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m))):
            return True
        else:
            return False
    elif is_planet(end(left(m))) == True and is_planet(end(right(m))) == False:
        if length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m))):
            return balanced(end(right(m)))
        else:
            return False
    elif is_planet(end(left(m))) == False and is_planet(end(right(m))) == True:
        if length(left(m)) * total_mass(end(left(m))) == length(right(m)) * total_mass(end(right(m))):
    ...
    if is_leaf(t):
        ...
        for branch in branches(t):
            ...
            if pruned_branch != None:
                new_brances.append(pruned_branch)
        return tree(label(t), new_brances)
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        summation = []
        for branch in branches(t):
            branch_sum = label(t) + max_path_sum(branch)
            summation.append(branch_sum)
